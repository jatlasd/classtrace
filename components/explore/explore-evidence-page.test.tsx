...
describe("ExploreEvidencePage", () => {
  ...
  it("reveals all available filters without running a query", () => {
    ...
    expect(screen.getByText(/What evidence did I save for Mary recently/)).toBeTruthy();
    expect(screen.getByText(/Which records have both #fractions and #independent/)).toBeTruthy();
    expect(screen.getByText(/Which students have #reteach evidence/)).toBeTruthy();
    expect(screen.getByText(/Evidence must match every filter you set/)).toBeTruthy();
    expect(screen.getByText(/it can match any one of them/)).toBeTruthy();
  ...
  it("opens and closes choices from the dropdown arrow and preserves selections", () => {
    renderPage();
    fireEvent.click(screen.getByRole("button", { name: "Show student choices" }));
    expect(screen.getByRole("listbox", { name: "Student choices" })).toBeTruthy();
    fireEvent.click(screen.getByRole("option", { name: /Mary/ }));
    expect(screen.getByRole("button", { name: "Remove Mary" })).toBeTruthy();
    expect(screen.getByLabelText("Student").getAttribute("placeholder")).toBe("Add another…");
    fireEvent.click(screen.getByRole("button", { name: "Show student choices" }));
    expect(screen.getByRole("option", { name: /Jeremy/ })).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Hide student choices" }));
    expect(screen.queryByRole("listbox", { name: "Student choices" })).toBeNull();
    expect(mocks.runExploreEvidenceQuery).not.toHaveBeenCalled();
  });
  ...
  it("keeps draft filters separate when changing the result view", async () => {
    ...
    renderPage();
    selectChoice("Student", "Mary");
    fireEvent.click(screen.getByRole("button", { name: "Group by student" }));
