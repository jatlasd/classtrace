import { FileDown, ListChecks, Tags, UsersRound } from "lucide-react";
...
const features = [
  ...
  },
  {
    ...
    body: "Teacher-reviewed types, topics, performance, tags, and follow-up. One evidence photo, without creating a file repository.",
  },
  {
    icon: FileDown,
...
export function LandingFeatures() {
  return (
    <section id="features" aria-labelledby="features-heading" className="scroll-mt-20 border-y border-border/70 bg-background">
      <div className="mx-auto grid max-w-[1180px] gap-10 px-4 py-14 md:px-6 lg:grid-cols-[0.72fr_1.28fr] lg:items-start lg:gap-20 lg:px-8 lg:py-20">
        <div className="max-w-sm">
          ...
          <p className="mt-4 text-[15px] leading-7 text-pretty text-muted-foreground">
        ...
        <ul className="min-w-0 border-t border-border">
          {features.map((feature) => (
            <li key={feature.title} className="grid grid-cols-[2rem_1fr] gap-4 border-b border-border py-4 sm:grid-cols-[2rem_0.7fr_1.3fr] sm:items-center sm:gap-5">
              ...
              <p className="col-start-2 min-w-0 text-sm leading-6 text-pretty text-muted-foreground sm:col-start-3">{feature.body}</p>
            </li>
          ))}
        </ul>
      </div>
